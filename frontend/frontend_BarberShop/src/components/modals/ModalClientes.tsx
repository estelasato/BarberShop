...
import {
  Cliente, criarCliente, atualizarCliente,
...
export function ModalClientes({
  ...
  async function carregarAux() {
    const [cid, est, cp] = await Promise.all([
  ...
  async function salvar() {
    ...
    if (cliente) await atualizarCliente(cliente.id, form as UpdateClienteDto)
    ...
    <>
      ...
      <Dialog open={isOpen} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-7xl w-full max-h-[90vh]">
          <DialogHeader>
            <DialogTitle>
              {cliente ? (readOnly ? "VISUALIZAR CLIENTE" : "EDITAR CLIENTE") : "NOVO CLIENTE"}
            </DialogTitle>
          </DialogHeader>
